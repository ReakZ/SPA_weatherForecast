...
class WeatherDisplay extends Component {
  ...
  render() {
    let properties = [
      { name: "temp", loc: "Температура", symbol: "°C" },
    ...
    return (
      <>
        {!dataWeather ? (
          <Spinner animation="border" role="status" style={{ margin: "50%" }}>
            <span className="sr-only">Loading...</span>
          </Spinner>
        ) : (
          <div>
            <h2>
              Погода в г.{this.props.city.name} <br />
            ...
            <Table striped bordered hover size="sm">
              <tbody>
                {properties.map((pro) =>
                  dataWeather[pro.name] ? (
                    <tr key={pro.name}>
                      <td>{pro.loc}:</td>
                      <td>{`${normalize(dataWeather[pro.name])} ${
                        pro.symbol
                      }`}</td>{" "}
                    </tr>
                  ) : null
